...
class Encoder_Model(pl.LightningModule):
    def __init__(self,depth, beta, tr_wei, param):
        ...
        if self.param.MM == True :
            if self.param.MM_type == 'param':
                if self.param.MM_fd_constr == False:
                    self.MM_out =  self.param.numOfMM*4
                else:
        ...
        if param.enc_type == 'conv':
            ...
            self.met2 = ConvNet_ENC(in_chanel=self.in_chanel, latent_Size=self.MM_out, dropout=self.dropout,
                                   freeze_enc=self.param.parameters["freeze_enc"],depth=2)
        ...
        if param.enc_type == 'trans':
            ...
            self.met = TransformerB(in_channels=1, out_channels=64, num_heads=4, hidden_size=64, num_layers=2,outsize=self.enc_out_)
            if param.parameters['decode'] == True:
    ...
    def model_decoder_met(self,enc):
        fr, damp,ph,ample_met,spline_coeff = self.get_model_parameters_met(enc)
        # damp = torch.clamp(damp, max=30)
        dec = self.lc_met(fr, damp,ph,ample_met)
        b_spline_rec = 0
        if self.param.parameters['spline']:
            b_spline_rec = self.bspline(spline_coeff)
        return fr, damp, ph, dec, ample_met, b_spline_rec

    def model_decoder_mm(self,enc_mm):
        ample_MM,mm_f,mm_damp, mm_phase = self.get_model_parameters_MM(enc_mm)
        # damp = torch.clamp(damp, max=30)
        mm_rec = torch.Tensor(0)
        if self.param.MM:
            mm_rec = self.lc_mm(ample_MM, mm_f, mm_damp, mm_phase)
        return mm_rec, ample_MM,mm_f,mm_damp, mm_phase
    def get_model_parameters_met(self,enc):
        ...
        return fr, damp,ph,ample_met,spline_coeff
    def get_model_parameters_MM(self,enc_mm):
        ample_MM = self.act(enc_mm[:, 0:self.param.numOfMM])
        mm_f = enc_mm[:, self.param.numOfMM:self.param.numOfMM*2]
        mm_phase = enc_mm[:, self.param.numOfMM*2:self.param.numOfMM*3]
        mm_damp = enc_mm[:, self.param.numOfMM*3:self.param.numOfMM*4]
            # torch.unsqueeze(enc[:, (self.param.numOfSig)+self.param.numOfMM*3],1)
        return ample_MM,mm_f,mm_damp, mm_phase
    def lc(self,enc):
        ...
        return out
    def lc_met(self,fr, damp,ph,ample_met):
        ...
        return dec
    def lc_mm(self, ample_MM,mm_f,mm_damp, mm_phase):
        if (self.param.MM == True):
                mm_rec = 0
                if self.param.MM_fd_constr == False:
        ...
        return mm_rec
    def forward(self, x):
        decoded_mm = self.param.inputSig(x,p1=0,p2=-1)
        enc_mm, _ = self.met2(decoded_mm)
        mm_rec, ample_MM,mm_f,mm_damp, mm_phase = self.model_decoder_mm(enc_mm)
        # x_ = x-mm_rec[:,0:x.shape[1]]
        decoded = self.param.inputSig(x)
        enc, latent = self.met(decoded)

        if self.param.MM_constr == True:
            self.tr = (self.sigm(enc[:,-1] - 5))
            fr, damp, ph, _, dec, ample_met, mm_phase,b_spline_rec = self.model_decoder_met(enc[:, 0:-1])
        else:
            fr, damp, ph, dec, ample_met, b_spline_rec = self.model_decoder_met(enc)
        ...
        if self.param.MM:
            ...
            dect = dec
        return dect, enc, ample_met, fr, damp, ph, mm_rec, dec, decoded,b_spline_rec
    ...
    def get_loss_freq(self,p1,p2,recons,input,b_spline_rec):
        if self.param.parameters['zero_fill'][0] == True:
    ...
    def loss_function(self,
        ...
        if self.param.MM:
            p1=0
            p2=-1
            loss_whole_range = self.get_loss_freq(p1,p2,recons,input,b_spline_rec)
        self.log("loss_whole_range", loss_whole_range)
        init_point = 1
        if self.param.parameters['fbound'][0]:
            ...
            recons_loss=self.get_loss_freq(p1,p2,recons,input,b_spline_rec)
        else:
        ...
        loss = met_loss + recons_loss + 0.01*loss_whole_range
    ...
    def training_step(self, batch, batch_idx):
        ...
        dec_real, enct, enc,_,damp,_,mm,dec,decoded,b_spline_rec = self(x)
        ...
        loss_mse,recons_loss = [lo/len(x) for lo in self.loss_function(dec, x, mu,logvar,mm,decoded,ampl_batch,enc,b_spline_rec)]
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if (self.current_epoch % self.param.parameters['val_freq'] == 0 and batch_idx == 0):
            ...
            rec_signal,_,enc, fr, damp, ph,mm_v,_,decoded, spline_rec = self(torch.unsqueeze(self.param.y_test_trun[id, :], 0).cuda())
    ...
    def bspline(self,coeff):
        ...
        t = torch.linspace(0, self.in_size, self.in_size).to(coeff.device)
