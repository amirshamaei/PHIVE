...
from Models.UNET_2d import ConvNet_ENC, ConvNet_DEC
import wandb
...
class Encoder_Model(pl.LightningModule):
    ...
    def lc_met(self,fr, damp,ph,ample_met,ample_MM,mm_f,mm_damp, mm_phase):
        ...
        dec = torch.multiply(sSignal, torch.exp(-2 * math.pi * ((fr).permute(0,2,1)) * self.t.T * 1j))
        dec = torch.multiply(dec, torch.exp((-1 * damp.permute(0,2,1)) * self.t.T))
        dec = (dec * torch.exp(ph.permute(0,2,1) * 1j))
        return dec
    ...
    def forward(self, x, cond = 0):
        ...
        enct = self.met.regres(latent).view(x.shape[0],64,-1)
        if self.beta != 0:
    ...
    def loss_function(self,
        ...
        if self.beta!=0:
            ...
            kld_loss = torch.mean(-0.5 * torch.sum(1 + std - std.exp(), dim=2), dim=(0,1))
    ...
    def validation_step(self, batch, batch_idx):
        ...
        try:
            if (self.current_epoch % self.param.parameters['val_freq'] == 0 and batch_idx == 0):
                ...
                plotppm(self.param,np.fft.fftshift(np.fft.fft((self.param.y_test_trun[id,32, :])).T)[p2:p1], rang[0], rang[1], False, linewidth=0.3, linestyle='-',label="y_test")
                ...
                rec_signal,_,enc, fr, damp, ph,mm_v,_,decoded, spline_rec,spline_coeff = self(self.param.y_test_trun[id, :,:].unsqueeze(0).cuda())
                spline_loss = torch.linalg.vector_norm(spline_coeff[:, :,:-1] - spline_coeff[:, :,1:])  #
                ...
                if self.param.parameters["decode"] == True:
                    if self.param.in_shape == 'real':
                        ...
                        plotppm(self.param,40+np.fft.fftshift(np.fft.rfft(
                            (decoded.cpu().detach().numpy()[0, 32, 0:self.param.truncSigLen])).T)[p2:p1], rang[0], rang[1],
                        ...
                        plotppm(self.param,40+np.fft.fftshift(np.fft.fft(
                            (decoded.cpu().detach().numpy()[0, 32,0:self.param.truncSigLen])).T)[p2:p1], rang[0], rang[1],
                                True, linewidth=1, linestyle='--',label="decoded")
                rec_sig = rec_signal.cpu().detach().numpy()[0, 32, 0:self.param.truncSigLen]
                ...
                if self.param.MM == True:
                    plotppm(self.param,5+10*np.fft.fftshift(np.fft.fft(((mm_v).cpu().detach().numpy()[0, 32, 0:self.param.truncSigLen])).T)[p2:p1], rang[0], rang[1], False, linewidth=1,linestyle='--',label="mm_v")
                ...
                if self.param.parameters['spline']:
                    ...
                    plt.plot(spline_rec[0,32].T)
                    # self.param.savefig(self.param.epoch_dir+"spline")
                    y_test = (self.param.y_test_trun[id, 32, :]).unsqueeze(0)
                    ...
                    plt.plot(torch.fft.fftshift(torch.fft.fft((rec_sig[0,32,:])).T)[self.p1:self.p2] + spline_rec[0,32].cpu().detach().T)
    ...
    def bspline(self,coeff):
        ...
        length, batch, num_sig = coeff.permute(2,0,1).shape
        coeff = coeff.view(-1,length)
        ...
        return (out.T).view(batch,num_sig,-1)
