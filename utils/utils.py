...
def cal_snrf(trnfreq,t_step,data_f,range=[2,4],endpoints=128,offset=0):
    p1 = int(ppm2p(trnfreq,t_step, range[0], data_f.shape[1]))
    p2 = int(ppm2p(trnfreq,t_step, range[1], data_f.shape[1]))
    return np.max(np.abs(data_f[:,p2:p1]), 1) / (np.std(data_f.real[:,offset:endpoints+offset],axis=1))

def ppm2p(trnfreq,t_step, r, len):
    r = 4.7 - r
...
def normalize(inp, axis=0):
    maxi = np.abs(inp).max(axis=0)
    return (np.abs(inp) / (maxi)) * np.exp(np.angle(inp) * 1j),maxi
# Function to load a NIfTI file
def load_nifti_file(file_path):
    # Load the NIfTI file
    nifti_img = nib.load(file_path)
    return nifti_img
def plotppm(sig, ppm1, ppm2, rev, linewidth=0.3, linestyle='-',label=None, mode='real',ax=None):
    # p1 = int(self.ppm2p(ppm1, len(sig)))
    # p2 = int(self.ppm2p(ppm2, len(sig)))
    # n = p2 - p1
    n = len(sig)
...
def plot_basis( trnfreq,t_step, basisset, t, met_name, ampl, fr, damp, ph, rng=[1,5]):
    reve = False
    # t = t.numpy()
    p1 = int(ppm2p(trnfreq,t_step,rng[0], len(basisset)))
    p2 = int(ppm2p(trnfreq,t_step,rng[1], len(basisset)))
    for i in range(0, len(basisset.T)):
        vv=fft.fftshift(fft.fft(ampl[0, i] * basisset[:len(t), i]*np.exp(-2 * np.pi *1j* fr * t.T)*np.exp(-1*damp*t.T)))
        if i ==len(basisset.T)-1:
            reve= True
        ax = plotppm(-4 * (i+2) + vv.T[p2:p1], rng[0], rng[1], reve)
        sns.despine(left=True,right=True,top=True)
        plt.text(.1, -4 * (i+2), met_name[i],fontsize=8)
