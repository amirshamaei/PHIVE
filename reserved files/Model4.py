...
class Encoder_Model(pl.LightningModule):
    def __init__(self,depth, beta, tr_wei, param):
        ...
        self.t = torch.from_numpy(param.t).float().cuda(self.param['gpu'])
        self.sw = 1/param.t_step
        self.basis = torch.from_numpy(param.basisset[0:2048, 0:param.numOfSig].astype('complex64')).cuda(self.param['gpu'])
        self.criterion = torch.nn.MSELoss(reduction='sum')
        self.beta = nn.Parameter(torch.tensor(beta), requires_grad=False).cuda(self.param['gpu'])
        # self.r2 = torchmetrics.R2Score(adjusted=True)
        if self.param.MM_constr == True :
            print('tr is not in the model')
        else:
            self.tr = nn.Parameter(torch.tensor(0.004).cuda(self.param['gpu']), requires_grad=True)
        self.tr_wei = tr_wei
        ...
        if self.param.MM == True :
            if self.param.MM_type == 'single' or self.param.MM_type == 'single_param':
                ...
                self.mm = torch.from_numpy(param.mm[0:2048].astype('complex64')).cuda(self.param['gpu']).T
        ...
        if self.param['spline']:
            self.enc_out_ = self.enc_out_ + self.param['numofsplines']
        try:
        ...
        if param.enc_type == 'conv':
            self.met = ConvNet_ENC(in_chanel=self.in_chanel,latent_Size=self.enc_out_, dropout=self.dropout,freeze_enc=self.param["freeze_enc"])
            if param.parameters['decode'] == True:
                self.decode = ConvNet_DEC(out_chanel=self.in_chanel,dropout=self.dropout, freeze_dec=self.param["freeze_dec"])
        if param.enc_type == 'trans':
            ...
            if param.parameters['decode'] == True:
                self.decode = ConvNet_DEC(out_chanel=self.in_chanel, dropout=self.dropout,
                                          freeze_dec=self.param["freeze_dec"])
        ...
        if self.param['zero_fill'][0] == True:
            self.param.truncSigLen = self.param['zero_fill'][1]
        if self.param['domain'] == 'freq':
            self.p1 = int(self.param.ppm2p(self.param['fbound'][2], (self.param.truncSigLen)))
            self.p2 = int(self.param.ppm2p(self.param['fbound'][1], (self.param.truncSigLen)))
    ...
    def model_decoder(self,enc):
        ...
        if self.param['zero_fill'][0] == True:
            temp = self.param.zero_fill_torch(temp, 1, self.param['zero_fill'][1])
    ...
    def get_model_parameters(self,enc):
        ...
        if self.param['spline']:
            spline_coeff = enc[:,
                      (self.param.numOfSig) + self.param.numOfMM * 4:self.param['numofsplines']+(self.param.numOfSig) + self.param.numOfMM * 4]
    ...
    def loss_function(self,
        ...
        init_point = 1
        div_fac = 1
        if self.param['fbound'][0]:
            if self.param['zero_fill'][0] == True:
                # recons = self.param.zero_fill_torch(recons,1,self.param['zero_fill'][1])
                input = self.param.zero_fill_torch(input, 1, self.param['zero_fill'][1])
            ...
            p1 = int(self.param.ppm2p(self.param['fbound'][2], (self.param.truncSigLen)))
            p2 = int(self.param.ppm2p(self.param['fbound'][1], (self.param.truncSigLen)))
        ...
        if self.param["supervised"] is True:
    ...
    def training_step(self, batch, batch_idx):
        if self.param["simulated"] is False:
            x = batch[0]
            ampl_batch = 0
        else:
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if (self.current_epoch % self.param['val_freq'] == 0 and batch_idx == 0):
            ...
            if self.param['spline']:
                ...
                y_test = self.param.zero_fill_torch(y_test, 1, self.param['zero_fill'][1]).cpu().detach()
                plt.plot(torch.fft.fftshift(torch.fft.fft((y_test)).T)[self.p1:self.p2])
                rec_sig = self.param.zero_fill_torch(rec_signal, 1, self.param['zero_fill'][1]).cpu().detach()
    ...
    def configure_optimizers(self):
        ...
        # if self.param['reduce_lr'][0] ==True:
        lr_scheduler = {
            # 'scheduler': torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min',patience=self.param['reduce_lr'][0]),
            ...
            'scheduler': torch.optim.lr_scheduler.MultiStepLR(optimizer,
                                                              ...
                                                              gamma=self.param['reduce_lr'][0]),
            # 'monitor':self.param['reduce_lr'][1],
            'name': 'scheduler'
        }
    ...
    def on_validation_epoch_end(self):
        r2 = []
        corr = []
        if self.param["simulated"] is True:
