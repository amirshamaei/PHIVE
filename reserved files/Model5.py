...
class Encoder_Model(pl.LightningModule):
    def __init__(self,depth, beta, tr_wei, param):
        ...
        if param.enc_type == 'conv':
            self.met = ConvNet_ENC(in_chanel=self.in_chanel+1,latent_Size=self.enc_out_, dropout=self.dropout,freeze_enc=self.param.parameters["freeze_enc"])
        ...
        self.embed = nn.Embedding(10, self.in_size)
    def sign(self,t,eps):
    ...
    def model_decoder(self,enc,condition):
        ...
        b_spline_rec = 0
        if self.param.parameters['spline']:
            b_spline_rec = self.bspline(spline_coeff,condition)
        if self.param.MM:
    ...
    def forward(self, x , condition):
        ...
        decoded = torch.concatenate((decoded,embed.unsqueeze(0).repeat(x.shape[0],1,1)),1)
        ...
        if self.param.MM_constr == True:
            ...
            fr, damp, ph, mm_rec, dec, ample_met, mm_phase,b_spline_rec = self.model_decoder(enc[:, 0:-1],condition)
        else:
            fr, damp, ph, mm_rec, dec,ample_met, mm_phase,b_spline_rec,spline_coeff = self.model_decoder(enc,condition)
    ...
    def loss_function(self,
        ...
        if self.param.parameters['fbound'][0]:
            ...
            spline_reg = torch.norm(spline_coeff)
            self.log("spline_reg", spline_reg)
        else:
        ...
        loss = met_loss + recons_loss + 2*condition*spline_reg
    ...
    def training_step(self, batch, batch_idx):
        ...
        condition = torch.randint(0,10,(1,)).to(x.device)
        dec_real, enct, enc,_,damp,_,mm,dec,decoded,b_spline_rec,spline_coeff = self(x,condition)
        ...
        loss_mse,recons_loss = [lo/len(x) for lo in self.loss_function(dec, x, mu,logvar,mm,decoded,ampl_batch,enc,b_spline_rec,spline_coeff,condition)]
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if (self.current_epoch % self.param.parameters['val_freq'] == 0 and batch_idx == 0):
            ...
            rec_signal,_,enc, fr, damp, ph,mm_v,_,decoded, spline_rec,spline_coeffs = self(torch.unsqueeze(self.param.y_test_trun[id, :], 0).cuda(),  torch.IntTensor([5]).cuda())
    ...
    def bspline(self,coeff,condition):
        # coeff_ = self.spline_net(torch.concatenate((coeff_,(condition.to(coeff_.device)).repeat(coeff_.shape[0]).unsqueeze(1)),1))
        # coeff = coeff_#[:,0:condition]
