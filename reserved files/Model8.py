...
class Encoder_Model(pl.LightningModule):
    ...
    def forward(self, x):
        ...
        embed_cond = self.one_hot[self.cond]
        enct = self.met.regres(torch.concatenate([latent.flatten(1),embed_cond],1))
    ...
    def loss_function(self,
        ...
        spline_coeff = spline_coeff * self.cond.unsqueeze(1).to(spline_coeff.device)
        spline_loss=torch.linalg.vector_norm(spline_coeff)#spline_coeff[:,:-1]-spline_coeff[:,1:])*2#
    ...
    def training_step(self, batch, batch_idx):
        ...
        # self.cond = random.randint(0,self.cond_max-1, (x.shape[0]))
        self.cond = torch.randint(0, self.cond_max, (x.shape[0],)).to(x.device)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        try:
            if (self.current_epoch % self.param.parameters['val_freq'] == 0 and batch_idx == 0):
                ...
                self.cond = torch.randint(0, self.cond_max, (1,)).to(x.device)
                ...
                self.log(f"spline_loss_val_{self.cond.cpu().detach()}", spline_loss)
                ...
                if self.param.parameters['spline']:
                    ...
                    plt.title(f'condition:{self.cond.cpu().detach()}')
