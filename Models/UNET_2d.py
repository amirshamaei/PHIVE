...
class BasicBlock_DOWN(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.conv1 = nn.Conv2d(out_channels, out_channels, kernel_size=kernel_size, padding=padding, stride=stride)
        ...
        self.downsample = nn.Conv2d(in_channels,out_channels,kernel_size=2, stride = 2, bias=False)
    def forward(self, x):
...
class ConvNet_ENC(nn.Module):
    ...
    def __init__(self, in_chanel, latent_Size,dropout=0,freeze_enc=False,depth=5):
        ...
        layers = []
        init = nn.Sequential(
            nn.Conv2d(in_chanel, 16, (2,5), stride=(1,2), padding=2),
            nn.Conv2d(16, 16, 5, stride=(1,2), padding=2))
        layers.append(init)
        ...
        self.reg = nn.Sequential(nn.Flatten(),
                                       nn.LazyLinear(latent_Size*64)) # TO DO: make it soft
        if freeze_enc==True:
